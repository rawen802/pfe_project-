...
def discover_one_device(device: dict, logger=None) -> dict:
    ...
    try:
        ...
        result = {
            "ip": device["ip"],
            "hostname": facts.get("hostname") or device.get("hostname"),
        ...
        print(f"[CDP] Neighbors found on {device['ip']}: {neighbors}")
...
def should_explore_neighbor(neighbor: dict) -> bool:
    ...
    if any(x in platform for x in allowed):
        return True

    if hostname.startswith(("sw", "r", "fw", "router", "switch")):
        return True
...
def discover_devices(seed_devices, logger=None) -> list:
    ...
    while queue:
        ...
        for neighbor in neighbors:
            print("[CDP Neighbor Found]:", neighbor)
            ...
            print("[QUEUE] Adding neighbor:", neighbor_ip)
