...
def enrich_rule_for_cisco(rule: dict):
    """
    Ajoute source_network, source_wildcard, destination_network, destination_wildcard
    """
...
def build_acl_device_map(acl_result: dict):
    """
    Regroupe les ACL par équipement.
    """
    device_map = {}
    ...
    for section in ["created", "updated", "deleted"]:
        for acl in acl_result.get(section, []):
            ...
            if section == "created":
                acl_copy["rules"] = [enrich_rule_for_cisco(rule) for rule in acl.get("rules", [])]

            elif section == "updated":
                acl_copy["new_rules"] = [enrich_rule_for_cisco(rule) for rule in acl.get("new_rules", [])]
...
def render_acl_configs_per_device(acl_result: dict, template_folder: str = "templates"):
    env = Environment(
...
def save_acl_configs_to_files(rendered_configs: dict, output_folder: str = "output/acl_configs"):
