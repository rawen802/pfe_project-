...
class AIAnalysisPage(QWidget):
    ...
    def send_apply_fix(self, fixes):
        if not fixes:
            QMessageBox.warning(self, "Aucune correction", "Aucune correction IA disponible.")
            return

        reply = QMessageBox.question(
            self,
            "Confirmation",
            "Voulez-vous vraiment appliquer ces corrections ?",
        ...
        payload = {"confirm": True, "fixes": fixes}

        try:
            response = requests.post(f"{API_URL}/ai/apply-fix", json=payload, timeout=30)

            if response.status_code == 200:
                QMessageBox.information(self, "Apply Fix", "Correction envoyée avec succès.")
            else:
                QMessageBox.critical(self, "Erreur API", response.text)
    ...
    def update_snapshot_from_report(self):
        ...
        device_count = summary.get("device_count", len(devices))
        vlan_count = summary.get("vlan_count") or len(network_context.get("vlans", [])) or self.count_vlans_from_devices(devices)
        acl_count = summary.get("acl_count") or summary.get("acl_candidate_count") or len(network_context.get("acls", [])) or self.count_acls_from_devices(devices)
