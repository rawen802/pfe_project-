...
class DeployAclPage(QWidget):
    ...
    def extract_acl_placement(self):
        """
        Essaie de récupérer l'emplacement ACL depuis plusieurs formats possibles.
        Compatible avec :
        - acl_plan["placement"]
        - acl_plan["recommended_placement"]
        - acl_plan directement avec device/interface/direction/reason
        - fallback : premier firewall/core détecté dans le report
        """
        ...
        interface = placement.get("interface") or placement.get("acl_interface") or plan.get("interface") or "-"
        direction = placement.get("direction") or plan.get("direction") or "IN"
        reason = placement.get("reason") or plan.get("reason") or "AI placement"
    ...
    def deploy_acl_config(self):
        ...
        try:
            ...
            if result.get("success"):
                self.handle_deploy_success(result.get("data", {}), hostname, ip)
            else:
                self.handle_deploy_error(result.get("error", "Erreur inconnue"), hostname, ip)
