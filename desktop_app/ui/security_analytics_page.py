...
class SecurityAnalyticsPage(QWidget):
    """
    Page Security Analytics prête pour l'utilisation réelle.

    Elle n'affiche plus de fausses valeurs fixes comme 100/100.
    Elle lit les vraies données depuis :
    - load_ai_analysis(analysis, report)
    - self.app_state["ai_validation"]
    - self.app_state["report"]
    - /ai/score-history/{user_id}
    """
    ...
    def setup_ui(self):
        ...
        summary_grid.addWidget(
            self.create_metric_card("Score de Sécurité Global", self.global_score_label, "/100", "Dernière analyse IA"),
            0, 0
        )
        summary_grid.addWidget(
            self.create_metric_card("Éléments analysés", self.elements_label, "", "Devices, VLANs, ACLs, liens"),
            0, 1
        )
        summary_grid.addWidget(
            self.create_metric_card("Problèmes détectés", self.issues_label, "", "Issues retournées par l’IA"),
            0, 2
        )
        summary_grid.addWidget(
            self.create_metric_card("Risques critiques", self.critical_label, "", "Sévérité HIGH"),
            0, 3
        )
        summary_grid.addWidget(
            self.create_metric_card("Dernière analyse", self.last_analysis_label, "", "État actuel"),
            0, 4
        )
    ...
    def create_header(self):
        ...
        subtitle = QLabel("Analyse statistique dynamique basée sur les vrais résultats IA et le vrai report réseau")
    ...
    # ======================================================
    # PUBLIC METHODS USED BY MAIN WINDOW / AI ANALYSIS PAGE
    # ======================================================
    ...
    def load_ai_analysis(self, analysis: dict, report: dict = None):
        """
        Appelée depuis AIAnalysisPage après une vraie validation IA.
        """
    ...
    def refresh_from_real_sources(self):
        """
        Bouton Actualiser : recharge les données réelles disponibles.
        Il ne crée pas de fausses données.
        """
        if isinstance(self.app_state, dict):
    ...
    def compute_component_scores(self):
        ...
        if ai_score is not None:
            try:
                scores["Global"] = max(0, min(100, int(ai_score)))
            except Exception:
                scores["Global"] = int(sum(scores.values()) / len(scores))
        else:
            scores["Global"] = int(sum(scores.values()) / len(scores))
    ...
    def estimate_elements_count(self):
        ...
        acls = network_context.get("acls", []) or []

        device_count = summary.get("device_count", len(devices)) or 0
        vlan_count = summary.get("vlan_count", len(vlans)) or 0
        subnet_count = summary.get("subnet_count", len(subnets)) or 0
        acl_count = summary.get("acl_count", summary.get("acl_candidate_count", len(acls))) or 0

        return int(device_count) + int(vlan_count) + int(subnet_count) + int(acl_count) + len(links)
    ...
    def apply_global_score_color(self, score):
        if score >= 80:
            ...
            color = "#ef4444"
        self.global_score_label.setStyleSheet(f"color: {color}; font-size: 28px; font-weight: 900;")
    ...
    def draw_empty_chart(self, chart, message):
        ...
        ax.text(0.5, 0.5, message, ha="center", va="center", color="#9DAEC8", fontsize=14, fontweight="bold")
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    def draw_component_chart(self, scores):
        ...
        values = [scores.get(component, 0) for component in components]
        ...
        for index, value in enumerate(values):
            ax.text(index, min(value + 3, 106), str(value), ha="center", va="bottom", color="white", fontsize=9, fontweight="bold")
    ...
    def draw_severity_chart(self, issues):
        ...
        high = sum(1 for issue in issues if isinstance(issue, dict) and str(issue.get("severity", "")).lower() == "high")
        medium = sum(1 for issue in issues if isinstance(issue, dict) and str(issue.get("severity", "")).lower() == "medium")
        low = sum(1 for issue in issues if isinstance(issue, dict) and str(issue.get("severity", "")).lower() == "low")

        values, labels, colors = [], [], []
        if high:
            values.append(high); labels.append("High"); colors.append("#ef4444")
        if medium:
            values.append(medium); labels.append("Medium"); colors.append("#facc15")
        if low:
            values.append(low); labels.append("Low"); colors.append("#22c55e")

        if not values:
            self.draw_empty_chart(self.severity_chart, "Aucun problème détecté")
            return

        ax.pie(values, labels=labels, autopct="%1.0f%%", startangle=90, colors=colors, textprops={"color": "white", "fontsize": 9})
        ax.set_facecolor("#10233F")
        fig.patch.set_facecolor("#10233F")
    ...
    def draw_history_chart(self, global_score=None):
        ...
        if not scores:
            ax.set_facecolor("#10233F")
            fig.patch.set_facecolor("#10233F")
            ax.text(0.5, 0.5, "Aucun historique disponible", ha="center", va="center", color="#9DAEC8", fontsize=14, fontweight="bold")
            ax.set_xticks([])
            ax.set_yticks([])
            for spine in ax.spines.values():
                spine.set_color("#1C3352")
    ...
    def load_score_history(self):
        ...
        try:
            ...
            response = requests.get(f"{API_URL}/ai/score-history/{user_id}", headers=headers, timeout=20)
            ...
            self.history_scores = [int(item.get("score", 0)) for item in history if isinstance(item, dict)]
