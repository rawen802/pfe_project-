...
class VlanVlsmPage(QWidget):
    ...
    def setup_ui(self):
        self.setStyleSheet("""
            ...
            QPushButton#OrangeBtn { background-color: #d97706; }
        ...
        self.existing_vlan_table = self.create_empty_table(
            ["VLAN ID", "Nom VLAN", "Zone", "Statut"], 210
        )
        self.existing_subnet_table = self.create_empty_table(
            ["Sous-réseau", "Masque", "Zone", "Utilisation"], 210
        )
        self.existing_zone_table = self.create_empty_table(
            ["Zone", "Type", "Description"], 210
        )
        self.switch_table = self.create_empty_table(
            ["Type", "Nom équipement", "Rôle"], 210
        )
    ...
    def export_vlan_vlsm_plan(self):
        """
        Exporte le plan final VLAN/VLSM en JSON.
        Ce bouton ne lance aucun déploiement : il sauvegarde uniquement le plan généré.
        """
        if not self.final_plan:
            QMessageBox.warning(
                self,
                "Plan vide",
                "Aucun plan VLAN/VLSM à exporter. Génère d'abord un plan final."
            )
            return
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Export réussi",
                f"Plan VLAN/VLSM exporté avec succès :\\n{path}"
            )
            ...
            QMessageBox.critical(
                self,
                "Erreur export",
                f"Impossible d'exporter le plan :\\n{e}"
            )
    ...
    def load_architecture_report(self):
        if not self.report_data:
            QMessageBox.warning(
                ...
                "Lance d’abord la découverte réseau. Ensuite le rapport sera envoyé automatiquement vers ce module."
            )
            return
    ...
    def populate_from_report(self, report):
        ...
        for device in devices:
            ...
            self.add_row_to_table(self.switch_table, [
                device_type,
                hostname,
                role
            ])
    ...
    def extract_vlsm_rows(self, data):
        if isinstance(data, dict):
            vlsm = data.get("vlsm", {})
            if isinstance(vlsm, dict):
                planned = vlsm.get("planned_subnets", [])
    ...
    def build_final_plan(self):
        ...
        for vlan in self.last_vlan_rows:
            ...
            for vlsm in self.last_vlsm_rows:
                ...
                vlsm_name = (
                    vlsm.get("departement")
                    or vlsm.get("zone_name")
                    or vlsm.get("zone")
                    or "-"
                )
    ...
    def generate_vlsm(self):
        ...
        if not self.report_data:
            QMessageBox.warning(
                self,
                "Rapport manquant",
                "Charge d’abord un rapport depuis le module Découverte Réseau."
            )
            return
        ...
        if not base_network or not zone or not hosts:
            QMessageBox.warning(
                self,
                "Champs manquants",
                "Veuillez remplir le réseau, la zone et le nombre d’hôtes."
            )
            return
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Erreur",
                "Le nombre d’hôtes doit être un entier."
            )
            return

        requirements = []

        if self.last_vlan_rows:
            for vlan in self.last_vlan_rows:
                if not isinstance(vlan, dict):
                    continue
                ...
                vlan_name = (
                    vlan.get("vlan_name")
                    or vlan.get("name")
                    or vlan.get("zone_name")
                    or vlan.get("zone")
                    or zone
                )

                required_hosts = vlan.get("required_hosts") or hosts
        ...
        vlsm_data = data.get("vlsm", {})
        rows = vlsm_data.get("planned_subnets", [])

        self.update_report_after_vlsm(rows)

        self.last_vlsm_rows = rows
    ...
    def generate_vlan(self):
        ...
        if not zone:
            QMessageBox.warning(
                self,
                "Champ manquant",
                "Veuillez entrer le nom de la zone."
            )
            return
